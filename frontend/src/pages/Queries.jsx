...
const Queries = () => {
                ...
                <CardContent>
                  ...
                  <Box sx={{ mt: 1 }}>
                    <Chip
                      ...
                      sx={{ mr: 0.5 }}
                    />
                    <Chip label={team.division} size="small" color="primary" />
                  </Box>
                </CardContent>
              </Card>
            </Grid>
            ...
            <TableBody>
              {data.map((row, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>
                    <Chip label={row.position} size="small" color="primary" />
                  </TableCell>
            ...
            <TableBody>
              {data.map((game, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>
                    <Chip label={game.home_team} color="primary" />
                  </TableCell>
                  <TableCell align="center">
                    <strong>
          ...
          <TableBody>
            {data.map((row, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell align="right">
                  {row.completion_pct}% ({row.completions}/{row.attempts})
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
          ...
          <TableBody>
            {data.map((row, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell align="right">{row.total_carries}</TableCell>
                <TableCell align="right">
                  <strong>{row.total_yards}</strong>
                </TableCell>
                <TableCell align="right">{row.total_tds}</TableCell>
                <TableCell align="right">{row.yards_per_game}</TableCell>
                <TableCell align="right">{row.yards_per_carry}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
          ...
          <TableBody>
            {data.map((row, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell>{row.team}</TableCell>
                <TableCell align="right">{row.games_played}</TableCell>
                <TableCell align="right">{row.total_receptions}</TableCell>
                <TableCell align="right">{row.total_targets}</TableCell>
                <TableCell align="right">
                  <strong>{row.total_yards}</strong>
                </TableCell>
                <TableCell align="right">{row.total_tds}</TableCell>
                <TableCell align="right">{row.yards_per_game}</TableCell>
                <TableCell align="right">{row.yards_per_catch}</TableCell>
                <TableCell align="right">{row.catch_rate}%</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
    ...
    return (
      <Box>
        <Typography variant="h6" sx={{ mb: 2 }}>
          {data.team} - {data.season} Season ({data.games_played} games)
        </Typography>
        <Grid container spacing={2}>
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="body2">
                  {data.passing.total_tds} TDs, {data.passing.total_ints} INTs
                </Typography>
              </CardContent>
            </Card>
          </Grid>
  ...
  const renderPlayerGameLog = (data) => {
    ...
    if (!data || !data.games || data.games.length === 0) {
      return <Alert severity="info">No game log found for this player</Alert>;
    }
            ...
            <TableBody>
              {data.games.map((game, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>{game.stadium || "N/A"}</TableCell>
                  <TableCell align="right">
